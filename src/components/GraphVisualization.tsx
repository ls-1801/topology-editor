...
import { Topology, SimulationNode, SimulationLink, NodeSelection } from '../types';
...
const GraphVisualization: React.FC<GraphVisualizationProps> = ({ topology, onNodeSelect }) => {
  ...
  useEffect(() => {
    ...
    topology.nodes.forEach(node => {
      ...
      if (node.physical && node.physical.length > 0) {
        ...
        node.physical.forEach((source, index) => {
          ...
          if (existingPos) {
            ...
            const angle = -Math.PI/4 - (Math.PI/2) * (index / (numPhysical > 1 ? numPhysical - 1 : 1));
      ...
      if (node.sinks && node.sinks.length > 0) {
        ...
        node.sinks.forEach((sink, index) => {
          console.log(`Creating sink node for ${node.connection} at index ${index}:`, sink);
          ...
          if (existingPos) {
            ...
            // Angle based on index (from 45 to 135 degrees in a semicircle)
            const angle = Math.PI/4 + (Math.PI/2) * (index / (numSinks > 1 ? numSinks - 1 : 1));
            const radius = 60; // Shorter fixed distance from main node
          ...
          nodes.push({
            ...
            parentId: node.connection,
            index: index // This must match the index in node.sinks array
          } as SimulationNode);
    ...
    topology.nodes.forEach(node => {
      if (node.links?.upstreams) {
        node.links.upstreams.forEach(sourceId => {
          ...
          const existingLink = mainLinks.find(link => 
    ...
    const simulation = d3.forceSimulation<SimulationNode>(nodes)
      .force('link', d3.forceLink<SimulationNode, SimulationLink>(links)
        ...
        .distance(d => {
          ...
          if (source.nodeType === 'physical' || target.nodeType === 'physical') return 80;
          if (source.nodeType === 'sink' || target.nodeType === 'sink') return 80;
    ...
    const link = g.append('g')
      ...
      .attr('stroke-width', d => {
        ...
        if (d.direction === 'downstream' || d.direction === 'upstream') return 2;
        return 1.5; // Thinner for physical/sink links
      })
      .attr('marker-end', d => {
        // Use different arrowheads for different link types
        if (d.direction === 'downstream' || d.direction === 'upstream') return 'url(#arrowhead-main)';
    ...
    node.append('circle')
      ...
      .attr('fill', d => {
        ...
        const capacityScale = d3.scaleLinear()
    ...
    function selectGraphNode(d: SimulationNode) {
      ...
      if (d.nodeType === 'main') {
        ...
        const actualNode = topology.nodes.find(node => node.connection === d.id);
        if (actualNode) {
          onNodeSelect({ type: 'main', nodeId: d.id });
        }
      } 
      // For physical source nodes
      else if (d.nodeType === 'physical' && d.parentId !== undefined && d.connection) {
        console.log(`Selecting physical source node: parentId=${d.parentId}, name=${d.connection}`);
        onNodeSelect({ 
          type: 'physical', 
          nodeId: d.id, 
          parentId: d.parentId, 
      ...
      // For sink nodes
      else if (d.nodeType === 'sink' && d.parentId !== undefined && d.connection) {
        console.log(`Selecting sink node: parentId=${d.parentId}, name=${d.connection}`);
        onNodeSelect({ 
          type: 'sink', 
          nodeId: d.id, 
          parentId: d.parentId, 
    ...
    function dragended(event: d3.D3DragEvent<SVGGElement, SimulationNode, SimulationNode>, d: SimulationNode) {
      ...
      if (d.nodeType === 'physical' || d.nodeType === 'sink') {
        ...
        for (const mainNode of mainNodes) {
          ...
          if (distance < 40) { // Using main node radius + some margin
